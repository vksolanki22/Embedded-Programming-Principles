...
void ssd1331_clear_screen_as_required(uint16_t hwColor,uint16_t iRowIndex,uint16_t iColumIndex)
{
	uint16_t i, j;

	for(i = iRowIndex; i < OLED_HEIGHT; i ++){
		for(j = iColumIndex; j < OLED_WIDTH; j ++){
			ssd1331_draw_point(j, i, hwColor);
		}
	}
}
